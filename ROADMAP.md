...
| ID | Item | Priority | Status | Notes |
|---|---|---|---|---|
| M1-C01 | Polars integration plugin | P1 | 🔄 IN PROGRESS | `attach_polars_guard()` hooks `collect`, `fetch`, `collect_async`, `sink_parquet`, `sink_csv`, `sink_ipc`, `sink_ndjson`; `install_polars_scan_budget()` enforces column-count and scan-node hard caps (raises) and soft caps (warn-log) before execution, now with native `LazyFrame.explain()` scan-node fallback parsing and custom `scan_count_fn` overrides; full native Polars callback integration remains. |
