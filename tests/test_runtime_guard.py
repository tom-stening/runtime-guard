...
class TestJsonEvents:
    ...
    def test_remote_wrapper_handles_method_maps_with_raising_setitem(self, monkeypatch):
        from runtime_guard import enable_ray_actor_memory_monitoring

        class _BrokenMethods(dict):
            def __setitem__(self, key, value):  # type: ignore[override]
                raise RuntimeError("broken record methods map setitem")
    ...
    def test_get_actor_report_global_view_normalizes_malformed_node_keys(self, monkeypatch):
        from runtime_guard import enable_ray_actor_memory_monitoring
        ...
        assert report["ok"] is True
        assert "unknown-node" in report["nodes"]
    ...
    def test_get_actor_report_handles_actor_rows_with_raising_get(self, monkeypatch):
        from runtime_guard import enable_ray_actor_memory_monitoring
        ...
        class _BrokenActorRow(dict):
            def get(self, key, default=None):  # type: ignore[override]
                raise RuntimeError("broken actor report actor row get")
    ...
    def test_get_actor_report_node_view_handles_node_rows_with_raising_get(self, monkeypatch):
        from runtime_guard import enable_ray_actor_memory_monitoring
        ...
        class _BrokenNodeRow(dict):
            def get(self, key, default=None):  # type: ignore[override]
                raise RuntimeError("broken actor report node-view row get")
...
class TestPolarsIntegration:
    ...
    def test_attach_chains_positional_callback_with_non_none_default(self, monkeypatch):
        sentinel = object()

        class CallbackPolars:
            class LazyFrame:
                def collect(self, optimization_callback: Any = sentinel) -> int:
                    if callable(optimization_callback):
                        optimization_callback("logical-plan")
                    return 42
    ...
    def test_attach_chains_callback_like_kwarg_with_kwargs_signature(self, monkeypatch):
        class CallbackPolars:
            class LazyFrame:
                def collect(self, multiplier: int = 1, **kwargs: Any) -> int:
                    callback = kwargs.get("optimization_callback")
                    if callable(callback):
                        callback("logical-plan")
                    return 21 * multiplier
    ...
    def test_attach_maps_alias_kwarg_into_positional_only_callback_when_var_kwargs_present(self, monkeypatch):
        class CallbackPolars:
            class LazyFrame:
                def collect(self, optimization_callback: Any | None = None, /, **kwargs: Any) -> int:
                    if callable(optimization_callback):
                        optimization_callback("logical-plan")
                    return 42
    ...
    def test_attach_maps_canonical_kwarg_into_positional_only_callback_when_var_kwargs_present(
        self, monkeypatch
    ):
        class CallbackPolars:
            class LazyFrame:
        ...
        try:
            user_calls: list[str] = []
            result = CallbackPolars.LazyFrame().collect(
                **{"optimization_callback": lambda plan: user_calls.append(plan)}
            )
            assert result == 42
            assert calls == ["polars-native", "polars-native-native-callback"]
            assert user_calls == ["logical-plan"]
    ...
    def test_attach_prefers_snake_case_when_exact_camel_param_is_passed(
        self, monkeypatch
    ):
        class CallbackPolars:
            class LazyFrame:
...
class TestDaskSchedulerCallbacks:
    ...
    def test_scheduler_callback_static_start_accepts_worker_alias_kwargs(self, monkeypatch):
        from runtime_guard import install_dask_scheduler_callbacks
        ...
        callback_cls.start("task-1", worker="worker-a")
        callback_cls.finish("task-1", "ok", worker_addr="worker-a")
    ...
    def test_scheduler_callback_context_accepts_worker_alias_kwargs(self, monkeypatch):
        from runtime_guard import install_dask_scheduler_callbacks
        ...
        ctx._pretask("task-1", worker_address="worker-a")
        ctx._posttask("task-1", "ok", worker="worker-a")
    ...
    def test_scheduler_callback_static_start_accepts_worker_alias_key_format_drift(self, monkeypatch):
        from runtime_guard import install_dask_scheduler_callbacks
        ...
        callback_cls.start("task-1", workerAddress="worker-a")
        callback_cls.finish("task-1", "ok", {"worker-id": "worker-a"})
    ...
    def test_scheduler_callback_static_start_accepts_hyphenated_worker_alias_kwargs(
        self, monkeypatch
    ):
        from runtime_guard import install_dask_scheduler_callbacks
        ...
        callback_cls.start("task-1", **{"worker-id": "worker-a"})
        callback_cls.finish("task-1", "ok", **{"worker-address": "worker-a"})
    ...
    def test_scheduler_callback_static_start_accepts_mapping_bytes_worker_alias_values(
        self, monkeypatch
    ):
        from runtime_guard import install_dask_scheduler_callbacks
        ...
        callback_cls.start("task-1", worker={"address": b"worker-a"})
        callback_cls.finish("task-1", "ok", worker_addr={"worker": b"worker-a"})
    ...
    def test_scheduler_callback_static_start_accepts_mapping_memoryview_worker_alias_values(
        self, monkeypatch
    ):
        from runtime_guard import install_dask_scheduler_callbacks
        ...
        callback_cls.start(
            "task-1",
            worker={"address": memoryview(b"worker-a")},
        )
        callback_cls.finish(
            "task-1",
            "ok",
    ...
    def test_get_worker_report_aggregate_handles_worker_rows_with_raising_get(self, monkeypatch):
        from runtime_guard import install_dask_scheduler_callbacks

        class _BrokenWorkerRow(dict):
            def get(self, key, default=None):  # type: ignore[override]
                raise RuntimeError("broken aggregate worker row get")
        ...
        assert aggregate_report["worker_details"]["worker-a"]["snapshots"] == []
        assert aggregate_report["parse_warning_count"] >= 1

    def test_scheduler_callbacks_handle_worker_rows_with_raising_get(self, monkeypatch):
        from runtime_guard import install_dask_scheduler_callbacks

        class _BrokenWorkerRow(dict):
            def get(self, key, default=None):  # type: ignore[override]
                raise RuntimeError("broken callback worker row get")
    ...
    def test_scheduler_callback_context_accepts_worker_alias_key_format_drift(self, monkeypatch):
        ...
        ctx._pretask("task-1", workerID="worker-a")
        ctx._posttask("task-1", "ok", workerAddr="worker-a")
    ...
    def test_scheduler_callback_static_start_accepts_worker_alias_positional_payloads(
        self, monkeypatch
    ):
        from runtime_guard import install_dask_scheduler_callbacks
    ...
    def test_scheduler_callback_static_start_accepts_positional_worker_id_string(
        self, monkeypatch
    ):
        from runtime_guard import install_dask_scheduler_callbacks
        ...
        callback_cls.start("task-1", "worker-a")
    ...
    def test_scheduler_callbacks_handle_worker_ids_with_raising_strip(self, monkeypatch):
        from runtime_guard import install_dask_scheduler_callbacks

        class _BadStr(str):
            def strip(self, chars=None):  # type: ignore[override]
                raise RuntimeError("broken worker id strip")
        ...
        callback_cls.start("task-1", worker_id=_BadStr("worker-a"))
        callback_cls.finish("task-1", "ok", worker_id=_BadStr("worker-a"))
    ...
    def test_scheduler_callbacks_handle_positional_worker_ids_with_raising_lower(
        self, monkeypatch
    ):
        from runtime_guard import install_dask_scheduler_callbacks
        ...
        class _BadStr(str):
            ...
            def lower(self):  # type: ignore[override]
                raise RuntimeError("broken worker id lower")
        ...
        callback_cls.start("task-1", _BadStr("worker-a"))
        callback_cls.finish("task-1", "ok", _BadStr("worker-a"))
    ...
    def test_scheduler_callbacks_handle_positional_worker_ids_with_raising_strip(
        self, monkeypatch
    ):
        from runtime_guard import install_dask_scheduler_callbacks
        ...
        class _BadStr(str):
            def strip(self, chars=None):  # type: ignore[override]
                raise RuntimeError("broken positional worker id strip")
    ...
    def test_scheduler_callback_static_start_accepts_positional_ucx_worker_endpoint(
        self, monkeypatch
    ):
        from runtime_guard import install_dask_scheduler_callbacks
        ...
        callback_cls.start("task-1", "ucx://10.0.0.1:13337")
        callback_cls.finish("task-1", "ok", "ucx://10.0.0.1:13337")

        worker_report = reporter("ucx://10.0.0.1:13337")
        assert worker_report["task_count"] == 1
        assert worker_report["completed_tasks"] == 1
        assert worker_report["healthy_events"] == 1
    ...
    def test_scheduler_callback_static_start_accepts_positional_bytes_worker_id(
        self, monkeypatch
    ):
        from runtime_guard import install_dask_scheduler_callbacks
        ...
        callback_cls.start("task-1", b"worker-a")
        callback_cls.finish("task-1", "ok", b"worker-a")
    ...
    def test_scheduler_callback_static_start_accepts_bytes_mapping_alias_keys(
        self, monkeypatch
    ):
        from runtime_guard import install_dask_scheduler_callbacks
        ...
        callback_cls.start("task-1", {b"workerAddress": "worker-a"})
        callback_cls.finish("task-1", "ok", {b"worker-id": "worker-a"})
    ...
    def test_scheduler_callback_static_start_accepts_memoryview_mapping_alias_keys(
        self, monkeypatch
    ):
        from runtime_guard import install_dask_scheduler_callbacks
        ...
        callback_cls.start("task-1", {memoryview(b"workerAddress"): "worker-a"})
        callback_cls.finish("task-1", "ok", {memoryview(b"worker-id"): "worker-a"})
    ...
    def test_scheduler_callback_context_accepts_worker_object_alias_values(self, monkeypatch):
        from runtime_guard import install_dask_scheduler_callbacks
        ...
        ctx._pretask("task-1", worker=_WorkerRef("worker-a"))
        ctx._posttask("task-1", "ok", worker_address=_WorkerRef("worker-a"))
    ...
    def test_scheduler_callback_static_start_accepts_nested_worker_alias_payloads(self, monkeypatch):
        ...
        callback_cls.start("task-1", worker={"address": "worker-a"})
        callback_cls.finish("task-1", "ok", worker_addr={"worker": "worker-a"})
    ...
    def test_scheduler_callback_context_accepts_nested_worker_alias_payloads(self, monkeypatch):
        from runtime_guard import install_dask_scheduler_callbacks
        ...
        ctx._pretask("task-1", worker={"worker_address": "worker-a"})
        ctx._posttask("task-1", "ok", worker={"worker": {"address": "worker-a"}})
    ...
    def test_scheduler_callback_static_start_accepts_sequence_worker_alias_payloads(
        self, monkeypatch
    ):
        from runtime_guard import install_dask_scheduler_callbacks
        ...
        callback_cls.start("task-1", [{"worker": "worker-a"}])
        callback_cls.finish("task-1", "ok", ({"worker_address": "worker-a"},))
    ...
    def test_scheduler_callback_static_start_handles_cyclic_sequence_worker_payload(
        self, monkeypatch
    ):
        from runtime_guard import install_dask_scheduler_callbacks
        ...
        payload.append({"worker": "worker-a"})

        callback_cls.start("task-1", payload)
        callback_cls.finish("task-1", "ok", payload)
    ...
    def test_scheduler_callback_static_start_handles_malformed_worker_id(self, monkeypatch):
        from runtime_guard import install_dask_scheduler_callbacks
        ...
        # Unhashable/non-string worker_id should fail safe to unknown-worker.
        callback_cls.start("task-1", worker_id=["worker-a"])
        callback_cls.finish("task-1", "ok", worker_id={"id": "worker-a"})
    ...
    def test_scheduler_callback_worker_report_handles_malformed_worker_id_lookup(self):
        ...
        # Lookup should not raise for malformed/unhashable worker ids.
        worker_report = reporter(["worker-a"])
        assert worker_report["ok"] is True
        assert worker_report["worker_id"] == "unknown-worker"
        assert worker_report["task_count"] == 0
        assert worker_report["completed_tasks"] == 0
        assert worker_report["pressure_events"] == 0
        assert worker_report["healthy_events"] == 0
    ...
    def test_scheduler_callback_write_path_recovers_from_malformed_worker_row(self, monkeypatch):
        ...
        # Should recover rather than raise when callback updates the same worker again.
        callback_cls.start("task-2", worker_id="worker-a")
        callback_cls.finish("task-2", "ok", worker_id="worker-a")
...
class TestRayActorMemoryMonitoring:
    ...
    def test_remote_wrapper_drops_kwargs_for_positional_only_node_actor_params(self, monkeypatch):
        ...
        wrapped = config["remote_wrapper"](compute)
        assert wrapped(10, node_id="node-posonly", actor_id="actor-posonly") == 11

        report = config["get_actor_report"](
            node_id="node-posonly",
            actor_id="actor-posonly",
        )
        assert report["ok"] is True
        assert report["events"] == 1
    ...
    def test_cluster_summary_handles_malformed_counter_fields(self, monkeypatch):
        from runtime_guard import enable_ray_actor_memory_monitoring
    ...
    def test_get_actor_report_sanitizes_malformed_actor_row_fields(self, monkeypatch):
        from runtime_guard import enable_ray_actor_memory_monitoring
        ...
        actor_row["events"] = "bad"
        actor_row["entry_checks"] = "bad"
        actor_row["exit_checks"] = "bad"
        actor_row["pressure_events"] = "bad"
        actor_row["healthy_events"] = "bad"
        actor_row["methods"] = "bad"

        report = config["get_actor_report"](node_id="node-a", actor_id="actor-1")
        assert report["ok"] is True
        assert report["events"] == 0
        assert report["entry_checks"] == 0
        assert report["exit_checks"] == 0
    ...
    def test_ray_report_paths_handle_node_keys_with_raising_strip(self, monkeypatch):
        from runtime_guard import enable_ray_actor_memory_monitoring

        class _BadStr(str):
            def strip(self, chars=None):  # type: ignore[override]
                raise RuntimeError("broken node key strip")
        ...
        all_nodes = config["get_all_node_reports"]()
        nodes = all_nodes["nodes"]
        node_row = nodes.pop("node-a")
        nodes[_BadStr("node-a")] = node_row

        global_report = config["get_actor_report"]()
        assert global_report["ok"] is True
        assert global_report["nodes_monitored"] >= 1
        assert global_report["parse_warning_count"] >= 1
    ...
    def test_cluster_summary_sanitizes_malformed_node_and_actor_keys(self, monkeypatch):
        ...
        summary = config["cluster_summary"]()
        assert summary["ok"] is True
        assert summary["busiest_node"] == "unknown-node"
        assert summary["busiest_actor"] == "actor-2"
        assert summary["actors_monitored"] == 1
        assert summary["busiest_actor_events"] == 3
        assert summary["parse_warning_count"] >= 3

    def test_get_all_node_reports_tracks_parse_warnings_for_malformed_node_keys(self, monkeypatch):
        from runtime_guard import enable_ray_actor_memory_monitoring
...
class TestPrometheusRenderer:
    ...
    def test_render_handles_boolean_metric_truthiness_raising(self):
        class _BadBool:
            def __bool__(self):
                raise RuntimeError("broken metric truthiness")

        report = _make_report(stage="metrics")
        report.is_critical = _BadBool()
        report.self_inflicted = _BadBool()
        text = render_prometheus_metrics(report)
...
class TestMultiProcessOrchestration:
    ...
    def test_make_worker_report_rejects_non_string_stage_and_worker_id(self, monkeypatch):
        ...
        with pytest.raises(ValueError, match="stage must be a string"):
            make_worker_report(guard, stage=1)  # type: ignore[arg-type]
    ...
    def test_make_worker_report_empty_worker_id_falls_back_to_pid(self, monkeypatch):
        ...
        report = make_worker_report(guard, stage="worker-a", worker_id="   ")
        assert report["worker_id"] == str(report["pid"])
    ...
    def test_aggregate_worker_reports_parse_warnings_include_invalid_worker_ids(self):
        summary = aggregate_worker_reports(
            [
                {"worker_id": "  ", "pressure": False, "severity": "none"},
                {"worker_id": 1, "pressure": False, "severity": "none"},
                {"worker_id": "ok", "pressure": False, "severity": "none"},
            ]
        )

        assert summary["parse_warning_count"] == 2
        assert summary["invalid_worker_id_workers"] == ["unknown-worker-1", "unknown-worker-2"]
