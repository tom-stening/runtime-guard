...
| M1-C07 | Config schema validation (`pydantic`) | P2 | 🔄 IN PROGRESS | Added optional schema validator with strict fallback; integrated posture validation into `make_pytest_guard(..., posture=...)` so repo-level pytest guard factories can apply validated presets without manual env plumbing; broader config-surface propagation remains. |
