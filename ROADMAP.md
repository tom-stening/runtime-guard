...
| M2-C02 | Audit log for all policy violations | P1 | 🔄 IN PROGRESS | Added hash-chained audit records + CLI verification, expanded taxonomy coverage for incident/access/integrity-style events, canonical normalization helpers (`audit_policy_taxonomy`, `normalize_policy_violation_event`) including action-triggered normalization when `event_type` is omitted, and optional dedup suppression on write-path (`append_audit_log(..., deduplicator=...)`) including signal-recovery policy support (`SIGNAL_RECOVERY_AUDIT_DEDUP_TTL_S`); broader policy catalog rollout remains. |
